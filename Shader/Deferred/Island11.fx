...
Texture2D<float> g_RefractionDepthTextureMS1;
...
static const float2 g_DiffuseTexcoordScale = { 130.0, 130.0 };
static const float2 g_RockBumpTexcoordScale = { 10.0, 10.0 };
static const float g_RockBumpHeightScale = 3.0;
static const float2 g_SandBumpTexcoordScale = { 3.5, 3.5 };
static const float g_SandBumpHeightScale = 0.5;
static const float g_TerrainSpecularIntensity = 0.5;
...
static const  float  g_DynamicTessFactor = 50.0f;
...
static const float g_ZNear = 0.1f;
...
{
    return  g_DynamicTessFactor * (1 / (0.015 * distance));
}
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
[domain("quad")]
PSIn_Diffuse HeightFieldPatchShadow(PatchData input,
                                    float2 uv : SV_DomainLocation,
                                    OutputPatch<DUMMY, 1> inputPatch)
{
    PSIn_Diffuse output;
...
[maxvertexcount(9)]
void CascadedShadowMapsGenDS(triangle PSIn_Diffuse InPos[3] : SV_Position, inout TriangleStream<GS_OUTPUT> OutStream)
{
      [unroll(3)]
    for (int iFace = 0; iFace < 3; iFace++)
    {
        ...
        for (int v = 0; v < 3; v++)
        {
           ...
           // int culled[4];
            ...
            output.Pos = mul(float4(InPos[v].position.xyz, 1.0f), CascadeViewProj[iFace]);
...
float4 WaterPatchPS(PSIn_Diffuse input) :SV_Target0
{
  ...
  // simulating scattering/double refraction: light hits the side of wave, travels some distance in water, and leaves wave on the other side
    ...
    scatter_factor +=  1.5 * g_WaterColorIntensity.y * max(0, input.positionWS.y + 1) *
    ...
    specular_factor =  fresnel_factor * pow(max(0, dot(pixel_to_light_vector, reflected_eye_to_pixel_vector)), g_WaterSpecularPower);
...
{
    float4 color;
    color.rgb = g_MainTexture.SampleLevel(SamplerLinearWrap, float2((input.texcoord.x - 0.5) / 1.1f + 0.5f, (input.texcoord.y - 0.5) / 1.1f + 0.5f), 0);
    color.a = 0;
...
{
    return g_RefractionDepthTextureMS1.Load(int3(input.position.xy, 0)).r;
}
...
//        SetDomainShader(NULL);
//        SetGeometryShader(NULL);
//        SetPixelShader(CompileShader(ps_5_0, RefractionDepthManualResolvePS2()));
//    }
//    pass P8
//    {
//        SetRasterizerState(NoCullMS);
...
//        SetDomainShader(NULL);
//        SetGeometryShader(NULL);
//        SetPixelShader(CompileShader(ps_5_0, RefractionDepthManualResolvePS4()));
//    }
