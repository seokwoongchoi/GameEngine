...
void DownScale4x42(uint2 CurPixel, uint groupThreadId)
{
    float avgLum = 0.0;
...
[numthreads(1024, 1, 1)]
void DownScaleFirstPass2(uint3 dispatchThreadId : SV_DispatchThreadID, uint3 groupThreadId : SV_GroupThreadID, uint3 groupId : SV_GroupID)
{
    uint2 CurPixel = uint2(dispatchThreadId.x % Res.x, dispatchThreadId.x / Res.x);

	// Reduce a group of 16 pixels to a single pixel and store in the shared memory
  DownScale4x42(CurPixel, groupThreadId.x);
...
Texture2D<float4> HDRDownScaleTex;
...
technique11 T0
{
    ...
    pass p2
    {
        ...
        SetComputeShader(CompileShader(cs_5_0, DownScaleFirstPass2()));
    }
    pass p3
    {
        SetVertexShader(NULL);
