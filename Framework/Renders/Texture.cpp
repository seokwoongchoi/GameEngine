...
D3D11_TEXTURE2D_DESC Texture::ReadPixel(DXGI_FORMAT format, vector<Color>* pixels)
{
	ID3D11Texture2D* srcTexture;
	view->GetResource((ID3D11Resource**)&srcTexture);

	return ReadPixel(srcTexture, format, pixels);
}

D3D11_TEXTURE2D_DESC Texture::ReadPixel(ID3D11Texture2D * src, DXGI_FORMAT format, vector<Color>* pixels)
{
	D3D11_TEXTURE2D_DESC srcDesc;
	src->GetDesc(&srcDesc);
...
TextureCube::TextureCube(Vector3 & position, UINT width, UINT height)
	:position(position), width(width), height(height), sView(nullptr), sProjection(nullptr)
{
	...
	//Create RTV
	{
		...
		D3D::GetDevice()->CreateRenderTargetView(rtvTexture, &desc, &rtv);
	...
	//Create SRV
	{
		...
		desc.TextureCube.MipLevels = maxMipLevels;
...
void TextureCube::Set(Shader * shader)
{
	...
	if (cubeMapBuffer == nullptr)
	{
		cubeMapBuffer = new ConstantBuffer(&cubeMapViewDesc, sizeof(CubeMapViewDesc));
		sCubeMapBuffer = shader->AsConstantBuffer("CB_CubeMap");
	}

	for (uint i = 0; i < 6; i++)
	{
		D3DXMatrixLookAtLH(&cubeMapViewDesc.CubeViews[i], &position, &direction[i], &up[i]);
	}
	

	
	perspective->GetMatrix(&cubeMapViewDesc.CubeProjection);
	

	cubeMapBuffer->Apply();
	sCubeMapBuffer->SetConstantBuffer(cubeMapBuffer->Buffer());
